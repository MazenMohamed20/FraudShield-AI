...
def plot_confusion_matrix(y_test, y_pred):
    ...
    plt.savefig(
        os.path.join(reports_path, "random_forest_confusion_matrix.png"),
        dpi=300,
        bbox_inches="tight"
    )
...
def plot_roc_curve(y_test, y_prob):
    ...
    plt.savefig(
        os.path.join(reports_path, "random_forest_roc_curve.png"),
        dpi=300,
        bbox_inches="tight"
    )
...
def save_metrics(
    ...
    with open(
        os.path.join(metrics_path, "random_forest_metrics1.txt"),
        "w"
    ) as file:
